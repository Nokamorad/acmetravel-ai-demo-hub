  ...
  return <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 shadow-md backdrop-blur-md py-4' : 'bg-transparent py-7'}`} data-pendo-id="main-navigation">
      <div className="container mx-auto flex items-center justify-between px-4 md:px-6">
        ...
        <div className="hidden md:flex items-center space-x-6">
          <a href="#onboarding" onClick={e => {
          e.preventDefault();
          smoothScroll('onboarding');
        }} className="text-midnight-navy hover:text-sky-blue transition-colors duration-200" data-pendo-id="nav-onboarding">
            Getting Started
          </a>
          <a href="#support" onClick={e => {
          e.preventDefault();
          smoothScroll('support');
        }} className="text-midnight-navy hover:text-sky-blue transition-colors duration-200" data-pendo-id="nav-support">
            Getting Help
          </a>
          <a href="#upsell" onClick={e => {
          e.preventDefault();
          smoothScroll('upsell');
          ...
          <Button variant="ghost" size="sm" onClick={toggleDemoMode} className="text-xs" data-pendo-id="toggle-demo-mode">
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && <div className="md:hidden bg-white shadow-lg absolute top-full left-0 w-full py-4 px-6 flex flex-col space-y-4">
          <a href="#onboarding" onClick={e => {
        e.preventDefault();
        smoothScroll('onboarding');
      }} className="text-midnight-navy hover:text-sky-blue transition-colors duration-200" data-pendo-id="mobile-nav-onboarding">
            Getting Started
          </a>
          <a href="#support" onClick={e => {
        e.preventDefault();
        smoothScroll('support');
      }} className="text-midnight-navy hover:text-sky-blue transition-colors duration-200" data-pendo-id="mobile-nav-support">
            Getting Help
          </a>
          <a href="#upsell" onClick={e => {
        e.preventDefault();
        smoothScroll('upsell');
          ...
          <Button variant="outline" size="sm" onClick={toggleDemoMode} className="text-xs w-full" data-pendo-id="mobile-toggle-demo-mode">
            {demoMode ? 'Exit Demo Mode' : 'Enter Demo Mode'}
          </Button>
        </div>}
    </nav>;
